...
<body>
      ...
      <h1>Book Now</h1>
        <form >
        <div class="text">
        <h3>Traveler Id.no:</h3>
                <input type="text" id="drno" name="drno" value=%s readonly required/>
        </div>
        ...
        <input type="submit" class="submit" id="sub" name="sub" value="Book">
...
if sub != None:
    q = """insert into travel(idno,fullname,age,gender, phonenumber,location, fromdate, todate, person ,address, city,state, pincode) 
    values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')""" % \
        (rno,fname, age, gender,phno,location, fdate, tdate, person, address, city, state, pin)
    ...
    location.href='b_login.py?rno=%s&';
    </script>
    """%(rno))
    conn.close()
